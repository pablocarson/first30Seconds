...
// GLOBAL FUNCTIONS
	// Global functions can occur on multiple pages or all pages.
 	...
 	// Firebase reference for server-generaged deviceID messages. These set the currently displayed page within the app.
	var currentPageRef = first30SecondsRef.child('global/serverEvents/currentPage');

	// Server sets newlyCreatedId to client
		currentPageRef.on('child_added', function(childSnapshot, prevChildName) {
	...
	// Server changes page to be displayed
		currentPageRef.on('child_changed', function(childSnapshot, prevChildName) {
			// Retrieve the JSON string stored in alertMsg	
			var val = childSnapshot.val();
			// If a message to change the current page is received, change to that page.	
			$.mobile.changePage("#" + val);					
		...
		globalServerAlertRef.on('child_added', function(childSnapshot, prevChildName) {
			...
			if (val.alertMsg == "") {
				...
				setTimeout (function() {
					...
					var containerHeightTrim = parseInt(containerHeight) + 37;			
					$('.alertWrapper').css("height", containerHeightTrim);
					// Profile page is a special case. We want to avoid a
				}, 10);		
			};		
		});
...
// HOME PAGE
	...
	var homeDataRef = first30SecondsRef.child('pages/home/pageData');

	// Server sends initial page data to client
		homeDataRef.on('child_added', function(childSnapshot, prevChildName) {
	...
	// Server changes page data
		homeDataRef.on('child_changed', function(childSnapshot, prevChildName) {
			// Close "Waiting..." overlay if it's open. If not, this command will be ignored.
			sys_closeWaiting();
			// Assign snapshot JSON to a variable
			var val = childSnapshot.val();
...
// INVITE PAGE
	...
	var inviteDataRef = first30SecondsRef.child('pages/invite/pageData');

	// Server sends initial page data to Firebase
		inviteDataRef.on('child_added', function(childSnapshot, prevChildName) {
		// Assign snapshot JSON to a variable
			var val = childSnapshot.val();
	...
	// Server changes page data
		inviteDataRef.on('child_changed', function(childSnapshot, prevChildName) {
		// Assign snapshot JSON to a variable
			var val = childSnapshot.val();
		// Display the page data from the snapshot to populate start time and 
		// distance from current location.
...
// INTRANSIT PAGE
	...
	var inTransitDataRef = first30SecondsRef.child('pages/inTransit/pageData')

	// Server sends initial page data to Firebase
		inTransitDataRef.on('child_added', function(childSnapshot, prevChildName) {
			// Assign snapshot JSON to a variable
			var val = childSnapshot.val();
	...
	// Server changes initial page data
		inTransitDataRef.on('child_changed', function(childSnapshot, prevChildName) {
...
// ATPARTY PAGE
	...
	// Firebase reference for page data for inTransit page
	var atPartyPauseRef = first30SecondsRef.child('pages/atParty/serverEvents/atPartyPauseMsg');

	// Listener: Server response to 'Pause Matches' request
		atPartyPauseRef.on('child_added', function(childSnapshot, prevChildName) {
			var val = childSnapshot.val();
	...
	// Listener: Server response to 'Pause Matches' request
		atPartyPauseRef.on('child_changed', function(childSnapshot, prevChildName) {
			var val = childSnapshot.val();
				...
				"<a href='#atPartyWaiting' id='link" + UID + "' data-rel='popup' data-position-to='window' onClick='usr_atPartyRateOtherUser(&#34;" + UID + "&#34; );'>" + 
					...
					"<div class='otherUserWrapper' id='div" + UID + "' style='hidden'>" + 
						...
						"<div class='description'><span class='description_content'>" + 
							...
							"<span id='ratingNumber' class='hidden'" + UID + "'>" + val.otherUserRatingMsg + "</span>" + 
	...
	// Firebase reference for 'Better Party' server messages
	var atPartyBetterPartyRef = first30SecondsRef.child('pages/atParty/serverEvents/betterParty');

	// Server sends a message to open or close a 'Better Party' overlay
		atPartyBetterPartyRef.on('child_added', function(childSnapshot, prevChildName) {
			var val = childSnapshot.val();
	...
	// Server sends a message to open or close a 'Better Party' overlay
		atPartyBetterPartyRef.on('child_changed', function(childSnapshot, prevChildName) {
			var val = childSnapshot.val();
			// If Better_party = true, open the 'Better Party' dialog
			if (val == true) {
				// Close the Waiting overlay if it's open
...
// RATEOTHERUSER PAGE
	...
	var rateOtherUserDataRef = first30SecondsRef.child('pages/rateOtherUser/pageData');

	// Server sends a unique ID which is used to populate the rateOtherUser page.
		rateOtherUserDataRef.on('child_added', function(childSnapshot, prevChildName) {
	...
	// Server sends a new unique ID which is used to repopulate the rateOtherUser page.
		rateOtherUserDataRef.on('child_changed', function(childSnapshot, prevChildName) {
			// Retrieve the unique ID from the Firebase message
			var UID = childSnapshot.val();
			// Use the unique ID and use it to identify the current instance of the page.
			$('#rateOtherUserUID').html( UID );
			// Retrieve the existing data from the thumbnail on the atParty page and populate the corresponding
			// rateOtherUser page component 
			var otherUserImage  = $('#image' + UID ).attr('src')
...
// MATCH PAGE
	// Firebase reference for page data for rateOtheruser page
	var matchDataRef = first30SecondsRef.child('pages/match/pageData');

	// Server sends a unique ID which is used to populate the match page.
		matchDataRef.on('child_added', function(childSnapshot, prevChildName) {
			// Retrieve the unique ID from the Firebase message
			var UID = childSnapshot.val();
			// Retrieve the existing data from the thumbnail on the atParty page and populate the corresponding
	...
	// Listener: Server sends another unique ID which is used to repopulate the match page.
		matchDataRef.on('child_changed', function(childSnapshot, prevChildName) {
			// Retrieve the unique ID from the Firebase message
			var UID = childSnapshot.val();
			// Retrieve the existing data from the thumbnail on the atParty page and populate the corresponding
			// Match page component 
...
// PROFILE PAGE
	// Firebase reference for page data for inTransit page
	var profileDataRef = first30SecondsRef.child('pages/profile/pageData');

	// Server sends existing profile data to Firebase
		profileDataRef.on('child_added', function(childSnapshot, prevChildName) {
			var val = childSnapshot.val();
	...
	// Server updates profile image
		profileDataRef.on('child_changed', function(childSnapshot, prevChildName) { 
		var val = childSnapshot.val();
...
// IMAGEUPLOAD PAGE
	...
	var imageUploadDataRef = first30SecondsRef.child('pages/imageUpload/pageData');
	...
	// Listener: imageUpload page data
		...
		imageUploadDataRef.on('child_added', function(childSnapshot, prevChildName) {
			var val = childSnapshot.val();
			console.log(val);
		// The imageUpload page uses the Cropit image upload plugin by Scott Cheng. This allows the user to open
		// an image from their device and then zoom, pan, crop, and upload it to Firebase. For API and details 
		...
		imageUploadDataRef.on('child_changed', function(childSnapshot, prevChildName) {
			var val = childSnapshot.val();
			console.log(val);
		// The imageUpload page uses the Cropit image upload plugin by Scott Cheng. This allows the user to open
		// an image from their device and then zoom, pan, crop, and upload it to Firebase. For API and details 
